...
type Interaction struct {
	...
	// RawRequest is the raw request received by the karmaecho server.
	RawRequest string `json:"raw-request,omitempty"`
	// RawResponse is the raw response sent by the karmaecho server.
...
type Options struct {
	...
	// Version is the version of karmaecho server
	Version string
	...
	EnableMetrics bool
	// ServerToken hide server version in HTTP response X-KarmaEcho-Version header
	NoVersionHeader bool
	// HeaderServer use custom string in HTTP response Server header instead of domain
	HeaderServer string

	// WebSocket configuration options
	// WSTimeout is the WebSocket connection timeout in seconds
	WSTimeout int
	// WSMaxConnections is the maximum connections per correlation ID
	WSMaxConnections int
	// WSMaxMessageSize is the maximum message size in bytes
...
// getURLIDComponent returns the karmaecho ID
