...
// Client is a client for communicating with karmaecho server instance.
type Client struct {
	busy                     sync.RWMutex
	State                    atomic.Value
...
// Options contains configuration options for karmaecho client
type Options struct {
	// ServerURL is the URL for the karmaecho server.
	ServerURL string
...
func New(options *Options) (*Client, error) {
	...
	// KARMAECHO_TLS_VERIFY (or INTERACTSH_TLS_VERIFY for backward compatibility)
	// enforces TLS (cleartext is a fatal error)
	if os.Getenv("KARMAECHO_TLS_VERIFY") == "true" || os.Getenv("INTERACTSH_TLS_VERIFY") == "true" {
		...
		karmaechoServerURL := options.ServerURL
		if !stringsutil.HasPrefixAnyI(karmaechoServerURL, "https://") {
			karmaechoServerURL = fmt.Sprintf("https://%s", karmaechoServerURL)
		}
		if _, err := httpclient.HTTPClient.Get(karmaechoServerURL); err != nil {
...
// StopPolling the karmaecho server.
...
func (c *Client) performRegistration(serverURL string, payload []byte) error {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		return errors.New("invalid token provided for karmaecho server")
	}
	if resp.StatusCode != http.StatusOK {
		data, _ := io.ReadAll(resp.Body)
