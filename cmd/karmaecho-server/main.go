...
import (
	...
	"github.com/karmagate/karmaecho/pkg/server/acme"
	"github.com/karmagate/karmaecho/pkg/settings"
	"github.com/karmagate/karmaecho/pkg/storage"
...
var (
	healthcheck           bool
	defaultConfigLocation = filepath.Join(folderutil.HomeDirOrDefault("."), ".config/karmaecho-server/config.yaml")
...
func main() {
	...
	flagSet.SetDescription(`KarmaEcho server - Go server to configure and host karmaecho OOB interaction server.`)
	...
	flagSet.CreateGroup("input", "Input",
		...
		flagSet.StringVar(&cliOptions.IPAddress, "ip", "", "public ip address to use for karmaecho server"),
		...
		flagSet.StringVarP(&cliOptions.OriginIPHeader, "origin-ip-header", "oih", "", "HTTP header containing origin ip (karmaecho behind a reverse proxy)"),
	)
	...
	flagSet.CreateGroup("config", "config",
		...
		flagSet.BoolVarP(&cliOptions.NoVersionHeader, "disable-version", "dv", false, "disable publishing karmaecho version in response header"),
	)

	flagSet.CreateGroup("update", "Update",
		flagSet.CallbackVarP(options.GetUpdateCallback("karmaecho-server"), "update", "up", "update karmaecho-server to latest version"),
		flagSet.BoolVarP(&cliOptions.DisableUpdateCheck, "disable-update-check", "duc", false, "disable automatic karmaecho-server update check"),
	)
	...
	flagSet.CreateGroup("services", "Services",
		...
		flagSet.BoolVarP(&cliOptions.RootTLD, "wildcard", "wc", false, "enable wildcard interaction for karmaecho domain (authenticated)"),
	...
	flagSet.CreateGroup("websocket", "WebSocket",
		flagSet.IntVarP(&cliOptions.WSTimeout, "ws-timeout", "wst", 60, "websocket connection timeout in seconds"),
		flagSet.IntVarP(&cliOptions.WSMaxConnections, "ws-max-conn", "wsmc", 10, "maximum websocket connections per correlation ID"),
		flagSet.IntVarP(&cliOptions.WSMaxMessageSize, "ws-max-msg", "wsmm", 1048576, "maximum websocket message size in bytes (default 1MB)"),
	)

	flagSet.CreateGroup("debug", "Debug",
		flagSet.BoolVar(&cliOptions.Version, "version", false, "show version of the project"),
		flagSet.BoolVar(&cliOptions.Debug, "debug", false, "start karmaecho server in debug mode"),
	...
	if !cliOptions.DisableUpdateCheck {
		latestVersion, err := updateutils.GetToolVersionCallback("karmaecho-server", options.Version)()
		if err != nil {
